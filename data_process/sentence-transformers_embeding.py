...
print(combined_t_features.shape)


#开始嵌入（mbert）
# 加载 SentenceTransformer 模型
# bert_model_dir = '/root/cxt/construct_graph_new/my-sentence-transformers-all-MiniLM-L6-v2'
# tokenizer = SentenceTransformer.from_pretrained(bert_model_dir)  # 加载预训练的mBERT分词器
# model = SentenceTransformer.from_pretrained(bert_model_dir)          # 加载预训练的mBERT模型

# model = SentenceTransformer("all-MiniLM-L6-v2")
model = SentenceTransformer('/root/cxt/construct_graph_new/my-sentence-transformers-all-MiniLM-L6-v2')
...
encoded_text = np.array(embeddings)
...
# np.save('/root/cxt/construct_graph_new/dataset/300dim_S-Transformers.npy', encoded_text_reduced)

# #
print("结合两个特征")
combined_features = np.concatenate((encoded_text_reduced, combined_t_features), axis=1)
np.save('/root/cxt/construct_graph_new/dataset/302dim_S-Transformers.npy', encoded_text_reduced)
print("结合后的维度数量：", combined_features.shape)
