...
# 初始化RoBERTa分词器和模型
model_name = "/root/cxt/construct_graph_20241125/my-MT5-base"
...
def encode_text(text):
    ...
    encode_text = outputs.last_hidden_state.mean(dim=1).squeeze().cpu().numpy()
    # encoded_text = outputs[0][:, 0, :].squeeze().cpu().numpy()
    encode_text = np.array(encode_text)
    print("encoded_text shape:", encode_text.shape)  # 打印形状，确保它是二维的
    # print("我走到很慢但我在走")
    return encode_text
...
np.save('/root/cxt/construct_graph_20241125/dataset/302dim_MT5.npy', combined_df)