...
class args_define():
    ...
    parser.add_argument('--n_infer_epochs', default=3, type=int,
    ...
    parser.add_argument('--data_path', default='/root/cxt/construct_graph_pair/graph_nowalk_newloss', #default='./incremental_0808/',
...
if __name__ == '__main__':
    ...
    with open(embedding_save_path + '/args.txt', 'w') as f:
        # 将args对象的所有属性以JSON格式写入到文件对象 f 中，使用了json.dump()函数。
        # args.__dict__返回args对象的属性字典，然后将其写入文件中。
        # 参数indent=2表示以缩进的方式将JSON数据写入文件，使其更易读。
    ...
    # Loss
    if args.use_hardest_neg:  # 是否使用最难三元组
    ...
    if args.use_dgi:
        # 模型的最后一层输出当作输入，未经过 Sigmoid 激活函数处理的结果，它结合了 Sigmoid 激活函数和二元交叉熵损失。
        # 在深度图像生成模型 (Deep Graph Infomax, DGI) 中，它用于衡量两个节点表示之间的相似度或关联程度。
    ...
    # 用于在恢复模型训练时执行特定的操作，比如从某个特定的训练点开始继续训练，或者在没有提供恢复路径时执行初始化操作等。
